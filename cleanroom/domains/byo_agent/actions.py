"""BYO-Agent actions — apply and rollback config deltas.

Actions maintain a stack of prior configs so rollback is exact round-trip.
The config is the ONLY mutable state; all other env keys (_agent, _eval, _grader, etc.)
are frozen.
"""
...
class BYOAgentActions:
    """Manages agent config state — apply and rollback via stack.
    ...
    def apply(self, env: dict, candidate: Candidate) -> None:
        ...
        Args:
            ...
            candidate: Proposed candidate with type="agent_config" and
                params={"config_delta": {...}}.

        Raises:
            ValueError: If candidate type is not "agent_config".
        """
        if candidate.type != "agent_config":
            raise ValueError(
                f"BYOAgentActions.apply: expected type='agent_config', got '{candidate.type}'"
            )
